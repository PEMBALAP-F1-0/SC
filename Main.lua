...
-- BEGIN: Inline Tabs (insert after aa:SetLanguage(ao.Language))
do
  -- Adds an inline tab bar + content manager to WindUI (aa)
  -- Usage after injection:
  -- local tabs = {
  --   { Id="yen", Title="Yen", Icon="currency", Build = function(parent) ... end },
  --   ...
  -- }
  -- local inline = aa.AddInlineTabs(windowObject, parentFrame, tabs, opts)
  -- inline:SetActive(index) -- set active programmatically
  ...
  function aa.AddInlineTabs(window, parent, tabs, opts)
    opts = opts or {}
    local corner = opts.Corner or 10
    local spacing = opts.Spacing or 8
    local buttonHeight = opts.ButtonHeight or 28
    local buttonPadding = opts.ButtonPadding or 8
    local iconSize = opts.IconSize or 16
    local activeIndex = opts.ActiveIndex or 1

    -- wrapper vertical -> bar + content
    ...
    local bar = NewRoundFrame(corner, "Squircle", {
      Size = UDim2.new(1, 0, 0, buttonHeight),
      ImageTransparency = 1,
      Parent = barHolder,
      ThemeTag = {
        ImageColor3 = "PopupBackground",
        ImageTransparency = "PopupBackgroundTransparency",
      },
    }, {
      New("UIPadding", {
        PaddingLeft = UDim.new(0, buttonPadding / 2),
        PaddingRight = UDim.new(0, buttonPadding / 2),
      }),
      New("UIListLayout", {
        FillDirection = "Horizontal",
        Padding = UDim.new(0, 6),
    ...
    -- content area (panels stacked; only active visible)
    ...
    local function setActive(index, instant)
      index = math.clamp(index or 1, 1, #tabs)
      for i = 1, #tabs do
        local btn = tabButtons[i]
        local cont = tabContents[i]
        if i == index then
          -- show active style
          if btn and btn.Frame then
            Tween(btn.Frame, 0.08, { ImageTransparency = 0 }):Play()
          end
          if cont then
            cont.Visible = true
          end
          ...
          if btn and btn.Frame then
            Tween(btn.Frame, 0.08, { ImageTransparency = 1 }):Play()
          end
          if cont then
            cont.Visible = false
          end
    ...
    for i, tab in ipairs(tabs) do
      ...
      local pill, pillAPI = NewRoundFrame(corner, "Squircle", {
        Size = UDim2.new(0, 0, 1, 0),
        ImageTransparency = 1,
        Parent = btnHolder,
        ThemeTag = { ImageColor3 = "Button" }
      }, {
        New("UIPadding", { PaddingLeft = UDim.new(0, buttonPadding), PaddingRight = UDim.new(0, buttonPadding) }),
        New("UIListLayout", { FillDirection = "Horizontal", Padding = UDim.new(0, 8), VerticalAlignment = "Center" })
      })

      -- optional icon
      if tab.Icon then
        local img = Creator.Image(tab.Icon, (tab.Title or "") .. ":" .. (tab.Icon or ""), 0, window.Folder or "Temp", "InlineTabs", true)
      ...
      tabButtons[i] = { Holder = btnHolder, Frame = pill, Label = label }
      ...
      -- connect click
      AddSignal(pill.MouseButton1Click, function()
        setActive(i, false)
      end)

      -- run builder if provided (safe pcall)
      if type(tab.Build) == "function" then
        spawn(function()
          local ok, err = pcall(function() tab.Build(content) end)
          if not ok then
            warn("[WindUI.AddInlineTabs] Build error for tab:", tab.Title, err)
          end
        end)
      end
    end
    ...
    local controller = {
      ...
      SetActive = function(_, idx) setActive(idx) end,
      GetActive = function() return activeIndex end,
